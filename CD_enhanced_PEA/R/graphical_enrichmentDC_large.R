...
for (i in 1:length(sim_enrichDC_large$simulations_complete)){
  data_list[[i]] = sim_enrichDC_large$simulations_complete[[i]]$data
  theta_list[[i]] = sim_enrichDC_large$simulations_complete[[i]]$theta
}
...
func <- function(x) {
  ...
  return(stability)}
clusterExport(cl = my.cluster, c("func","data_list", "theta_list"))
...
save(output, file = "../data/Graphical_enrichDC_large.RData")