...
sim_small_top_sparse <- small_topology[[2]]

data_list = NULL
theta_list = NULL

for (i in 1:length(sim_small_top_sparse)){
  data_list[[i]] = sim_small_top_sparse[[i]]$data
  theta_list[[i]] = sim_small_top_sparse[[i]]$theta
}
...
save(output, file = "../data/Graphical_top_small_sparse.RData")